...
const Adminformsignup = () => {
  
  const [type, setType,] = useState("password");
  const [ type1, setType1,] = useState('password')
  return (
    <div className='bgimg w-[100%] h-[200%] fixed right-0 ' style={{backgroundSize:'100% 100vh'}} >
      <div className='adform w-[80%] sm:w-[60%] md:w-[50%] h-[500px] block my-20 rounded-2xl m-auto lg:flex lg:w-[70%] bg-coral bg-opacity-60 '>
        <form action="" className='lg:w-[50%] '>
          <h1 className='text-3xl font-bold text-white p-5 '>Create account</h1>
          ...
          <label htmlFor="passkey" className='text-lg p-4 font-semibold' >Confirm password <span className='text-red '>*</span></label>
          <div className="flex flex-row ml-2 ">
            <input className='pl-2 my-3 mx-auto block h-[50px] w-[95%] rounded-3xl outline-none text-sm ' type={type} name="password" id="" placeholder='Confirm password' />
            {type1 === "password" ? (
              <FaRegEyeSlash onClick={() => setType1("text")} className='my-7 relative right-[13%]' />
            ) : (
              <FaRegEye onClick={() => setType1("password")} className='my-7 relative right-[13%]' />
            )}
          </div>
          <div className="btn">
        ...
        <div className=" my-6 ">
        <span className='block text-center font-semibold '>Already have an account? <Link to='/admin/login' className=' font-semibold text-white  mr-4'>Login</Link></span>
        </div>
        </form>
...
export default Adminformsignup;
